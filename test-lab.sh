...
if [[ $START_PHASE -le 1 ]]; then
  ...
  # 1e. Webserver reachable from NVA1 internal NIC
  log "Checking webserver reachability from NVA1..."
  if $SSH1 "curl -sk --max-time 5 https://${WEBSERVER_IP}/healthz -o /dev/null -w '%{http_code}'" | grep -q "200"; then
    pass "Webserver responds 200 on /healthz (via NVA1 internal NIC)"
  else
...
if [[ $START_PHASE -le 3 ]]; then
  ...
  log "Active conntrack entries on NVA1 (webserver flows):"
  $SSH1 "sudo conntrack -L 2>/dev/null | grep ${WEBSERVER_IP} || echo '  (none)'"
  echo ""

  log "Active conntrack entries on NVA2 (webserver flows):"
  $SSH2 "sudo conntrack -L 2>/dev/null | grep ${WEBSERVER_IP} || echo '  (none)'"
  echo ""

  echo "Look for INVALID DROP hits in the FORWARD chain above."
  echo "An NVA with INVALID drops but fewer/no conntrack entries for ${WEBSERVER_IP}"
  echo "is the one receiving asymmetric return traffic."
fi
