...
#
# Frontend:    public IP, HTTPS:443 listener with self-signed cert.
# Backend pool: Internal LB private frontend (10.0.2.4).
# Backend HTTP setting: HTTP:80 (lab simplification — production should
#   use HTTPS:443 end-to-end with the webserver cert added as a
#   trusted_root_certificate).
# Probe: HTTP /healthz with Host: connect.clientmworkspace.com.
# ---------------------------------------------------------------------------

# Self-signed cert for the listener
...
resource "azurerm_application_gateway" "main" {
  ...
  # Backend pool = Internal LB frontend IP. App GW connects here, the LB
  # forwards to whichever NVA wins the SourceIP hash, and the connection
  # is symmetric for the lifetime of the flow.
  backend_address_pool {
    name         = "bepool-internal-lb"
    ip_addresses = [var.internal_lb_frontend_ip]
  }

  backend_http_settings {
    name                                = "settings-internal-lb"
  ...
  request_routing_rule {
    name                       = "rule-https-to-internal-lb"
    priority                   = 1
    rule_type                  = "Basic"
    http_listener_name         = "listener-https-public"
    backend_address_pool_name  = "bepool-internal-lb"
    backend_http_settings_name = "settings-internal-lb"
  }
}
