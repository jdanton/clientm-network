...
| [`proposed-working-design-5/`](proposed-working-design-5/) | **Two-firewall solution** — design-4's single 3-NIC NVA for inbound + Azure Firewall for egress. Answers "the client needs both firewall products in the architecture." NGFW-shape inbound preserved; FQDN-aware egress restored. ~$1,307/mo. **Deployed and verified end-to-end (7/7)**; also retested with a **dual-frontend (public + private IP) App Gateway** — see [VERIFIED.md](proposed-working-design-5/VERIFIED.md) and the [network diagram](proposed-working-design-5/network-diagram.svg). |
| [`proposed-working-design-6/`](proposed-working-design-6/) | **design-5 with a private front door** — the App Gateway is fronted by a **private IP** (`10.0.1.10`) instead of a public one, for an internal app with no public exposure. Same two-firewall inspection chain behind it. Ships an `appgw_private_only` toggle (private-primary + public fallback by default; flip to true private-only once the network-isolation feature is registered). **Deployed and verified end-to-end (7/7)**. ~$1,371/mo — see [README.md](proposed-working-design-6/README.md) and the [network diagram](proposed-working-design-6/network-diagram.svg). |
...
| [design-6](proposed-working-design-6/README.md#cost-us-east-247) | design-5 with a **private-IP App Gateway front door** (internal app, no public exposure) + NAT GW | yes (FQDN) | **~$1,371** |
