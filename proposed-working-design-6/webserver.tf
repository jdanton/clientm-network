...
resource "azurerm_linux_virtual_machine" "webserver" {
  ...
  # Boot the webserver after the NVA (inbound path) AND the Azure Firewall's
  # rule collection + the DMZ 0.0.0.0/0 → firewall UDR (egress path), so the
  # webserver's first apt hits a configured firewall with rules in place.
  # Combined with the cloud-init apt retry loop, this avoids the rule-
  # propagation race design-3 first hit.
  depends_on = [
