...
# Public IP exists ONLY as a lab-reachability fallback. When
# appgw_private_only = true it is not created at all — the App Gateway is then
# truly private (which requires the network-isolation feature on the sub).
resource "azurerm_public_ip" "appgw" {
  count               = var.appgw_private_only ? 0 : 1
...
resource "azurerm_application_gateway" "main" {
  ...
  # design-6: the PRIVATE frontend is the primary front door. Clients reach the
  # App Gateway over 10.0.1.10 from inside the VNet, a peered VNet, or on-prem
  # via VPN/ExpressRoute — never the public Internet.
  ...
  # Public frontend — retained ONLY as a lab-reachability fallback, and dropped
  # entirely when appgw_private_only = true. (A truly private-only WAF_v2
  # gateway needs Microsoft.Network/EnableApplicationGatewayNetworkIsolation.)
  dynamic "frontend_ip_configuration" {
    for_each = var.appgw_private_only ? [] : [1]
    content {
  ...
  # Primary rule: the private front door.
  ...
  # Fallback rule for the public frontend — omitted in private-only mode.
  dynamic "request_routing_rule" {
    for_each = var.appgw_private_only ? [] : [1]
    content {
      name                       = "rule-https-public-to-nva-trust"
      priority                   = 2
      rule_type                  = "Basic"
